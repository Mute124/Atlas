...
namespace Atlas {
	/**
	 * @brief Represents an object that can be rendered in the screen and contains
	 * all the common properties of a game object.
	 * 
	 * @remarks 
	 * 
	 * @paragraph gameobjects-interface-uses Class Use Cases
	 * You may implement this yourself to create your own game objects and drawing
	 * code, or you can inherit from @ref GameObject to use the default implementation.
	 * However, if you inherit from this class, be sure you have a basic understanding
	 * of Raylib's and OpenGL's rendering system. If not, see @link https://learnopengl.com 
	 * @endlink
	 * or @link https://www.raylib.com @endlink for a good place to start. In addition, 
	 * it is not advised for you to inherit this yourself because it will require you to 
	 * implement rendering code. However, it @a is recommended if your game requires a
	 * rendering system that is not provided by default. If you do implement your own
	 * rendering system, be sure to ensure that the @ref Renderer used in the @ref IAtlasEngine
	 * derived class or @ref AtlasEngine class can be used with your code.
	 * @endparblock
	 * 
	 * @note The IGameObject class is abstract and cannot be instantiated directly.
	 * Instead, you should inherit from this class and implement the pure virtual
	 * functions. As of version 0.0.3, the pure virtual functions are: @ref render(),
	 * @ref texture(), @ref update(), @ref prePhysicsUpdate(), @ref postUpdate(),
	 * @ref destroy(), and @ref cleanup().
	 * 
	...
	protected:
		...
		/**
		 * @brief Represents an exception that is thrown when a component name ends up being
		 * invalid.
		 * 
		 * @since v0.0.9
		 * 
		 ...
		 * 
		 * @remarks This is done by using the typeid of @ref T_COMPONENT_TYPE.
		 * This is done to ensure that the component name is unique and that it
		 * is not the same as another component name. Furthermore, it is done to
		 * make it easier to identify the type of the component without having to
		 * manually define a string for each component.
		 * 
	...
	public:
		...
		/**
		 * @brief Represents an exception that is thrown when a game object is a
		 * nullptr. This is a final subclass of @ref InvalidGameObjectException
		 * 
		 * @since v0.0.9
		 * 
		 ...
		 * @brief Adds a component to this game object
		 * 
		 * @remarks The component is added by making a shared pointer of the
		 * @ref component parameter and assigning it to the component name in @ref mComponents.
		 * 
		 * @throws InvalidComponentNameException if the component name is empty
		 * 
		 * @tparam T_COMPONENT_TYPE The type of the component being added.
		 ...
		 * @tparam T_COMPONENT_TYPE The type of the component being added.
		 * 
		 * @tparam ...Args A variadic parameter pack of arguments to be forwarded
		...
		void addComponent(Args... args) {
			...
			mComponents[componentName] = std::make_shared<T_COMPONENT_TYPE>(new T_COMPONENT_TYPE(args)...);
			mComponents[componentName]->setOwner(this);
		}
		...
		/**
		 * @brief Prepares this instance for the main update. This is a @b pure virtual function
		 * and @b must be implemented in order for the object to be updated
		 * 
		...
		/**
		 * @brief The main update function. This is a @b pure virtual function and @b must be
		 * implemented in order for the object to be updated
		 * 
		 * @since v0.0.1
		 */
		...
		/**
		 * @brief Anything that needs to be done after the main update. This is a pure virtual
		 * function and must be implemented in order for the object to be updated.
		 * 
		 * @since v0.0.1
		 */
		...
		/**
		 * @brief A pure virtual function that is the drawing function for when objects are
		 * being textured to the render texture.
		 * 
		 * @remarks If you do not have anything different to do here, then you may just call
		 * @ref render() instead, but this must be implemented in order for the object to be 
		 * rendered because the rendering system draws game objects to a render texture.
		 * 
		 * @since v0.0.1
		 */
		...
		/**
		 * @brief A pure virtual function that is the rendering function that will be called
		 * @b AFTER the render texture has been drawn to the screen.
		 * 
		 * @remarks You can draw objects within this function, but behavior will be undefined
		 * if you do so.
		 * 
		 * @since v0.0.1
		 */
		...
		/**
		 * @brief A pure virtual function that is called when the object is being destroyed.
		 * This happens right before the object is removed from memory.
		 * 
		 * @since v0.0.1
		 */
